...
# throughout the cell. Although mutations are technically mistakes, a very rare mutation may equip the 
# cell with a beneficial attribute. In fact, the macro effects of evolution are attributable by the 
# accumulated result of beneficial microscopic mutations over many generations.
...
class DNA
  ...
  def hamming_distance(string)
    strand1, strand2 = [string, @strand].sort { |a, b| a.size <=> b.size }
    count = 0
    strand1.chars.each_index do |index|
